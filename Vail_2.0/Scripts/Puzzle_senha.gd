...
func _process(delta):
	if sequencia == senha and not isCerto:
		isCerto = true
	...
	if pilar == 4 and sequencia != senha:
		...
		$Pilar2.get_node("Pilar_Area_1").disabled = false
		$Pilar3.get_node("Pilar_Area_1").disabled = false
		sequencia = 0
		pilar = 1
...
func _on_Pilar_area_entered(area):
	if not $Pilar.get_node("Pilar_Area_1").disabled :
		sequencia += pilar * 2
		pilar +=1
		$Pilar/AnimationPilar.play("Ativado")
		$Pilar.get_node("Pilar_Area_1").disabled = true
	pass 


func _on_Pilar2_area_entered(area):
	if not $Pilar2.get_node("Pilar_Area_1").disabled:
		sequencia += pilar * 4
		pilar+=1
		$Pilar2.get_node("AnimationPilar").play("Ativado")
		$Pilar2.get_node("Pilar_Area_1").disabled = true
	pass 


func _on_Pilar3_area_entered(area):
	if not  $Pilar3.get_node("Pilar_Area_1").disabled :
		sequencia += pilar * 8
		pilar+=1
		$Pilar3.get_node("AnimationPilar").play("Ativado")
		$Pilar3.get_node("Pilar_Area_1").disabled = true
